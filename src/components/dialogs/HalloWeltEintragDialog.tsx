...
interface HalloWeltEintragDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (fields: HalloWeltEintrag['fields']) => Promise<void>;
  defaultValues?: HalloWeltEintrag['fields'];
  enablePhotoScan?: boolean;
  enablePhotoLocation?: boolean;
}

export function HalloWeltEintragDialog({ open, onClose, onSubmit, defaultValues, enablePhotoScan = true, enablePhotoLocation = true }: HalloWeltEintragDialogProps) {
  const [fields, setFields] = useState<Partial<HalloWeltEintrag['fields']>>({});
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSaving(true);
    try {
      const clean = cleanFieldsForApi({ ...fields }, 'hallo_welt_eintrag');
      await onSubmit(clean as HalloWeltEintrag['fields']);
      onClose();
    } finally {
      setSaving(false);
    }
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      setFields(prev => {
        ...
        return merged as Partial<HalloWeltEintrag['fields']>;
      });
      setAiText('');
      setScanSuccess(true);
  ...
  const DIALOG_INTENT = defaultValues ? 'Hallo Welt Eintrag bearbeiten' : 'Hallo Welt Eintrag hinzufügen';
        ...
        <form onSubmit={handleSubmit} className="flex flex-1 flex-col min-h-0">
          ...
          <div className="space-y-1.5">
            ...
            <Input
              ...
              type="number"
              value={fields.nummer ?? ''}
              onChange={e => setFields(f => ({ ...f, nummer: e.target.value ? Number(e.target.value) : undefined }))}
            />
          </div>
          <div className="space-y-1.5">
