...
export default function HalloWeltEintragPage() {
  const [records, setRecords] = useState<HalloWeltEintrag[]>([]);
  const [loading, setLoading] = useState(true);
  const [search, setSearch] = useState('');
  const [dialogOpen, setDialogOpen] = useState(false);
  const [editingRecord, setEditingRecord] = useState<HalloWeltEintrag | null>(null);
  const [deleteTarget, setDeleteTarget] = useState<HalloWeltEintrag | null>(null);
  const [viewingRecord, setViewingRecord] = useState<HalloWeltEintrag | null>(null);
  ...
  async function loadData() {
    setLoading(true);
    try {
      setRecords(await LivingAppsService.getHalloWeltEintrag());
    } finally {
      setLoading(false);
    }
  }

  async function handleCreate(fields: HalloWeltEintrag['fields']) {
    await LivingAppsService.createHalloWeltEintragEntry(fields);
    await loadData();
    setDialogOpen(false);
  }

  async function handleUpdate(fields: HalloWeltEintrag['fields']) {
    if (!editingRecord) return;
    await LivingAppsService.updateHalloWeltEintragEntry(editingRecord.record_id, fields);
    await loadData();
  ...
  return (
    <PageShell
      title="Hallo Welt Eintrag"
      subtitle={`${records.length} Hallo Welt Eintrag im System`}
      action={
    ...
    >
      <div className="relative w-full max-w-sm">
        ...
        <Input
          placeholder="Hallo Welt Eintrag suchen..."
          value={search}
            ...
            {filtered.length === 0 && (
              <TableRow>
                <TableCell colSpan={4} className="text-center py-16 text-muted-foreground">
                  {search ? 'Keine Ergebnisse gefunden.' : 'Noch keine Hallo Welt Eintrag. Jetzt hinzufügen!'}
                </TableCell>
              </TableRow>
            )}
          </TableBody>
      ...
      <ConfirmDialog
        ...
        title="Hallo Welt Eintrag löschen"
