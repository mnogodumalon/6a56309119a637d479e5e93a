...
import { HalloWeltEintragViewDialog } from '@/components/dialogs/HalloWeltEintragViewDialog';
...
const HALLOWELTEINTRAG_FIELDS = [
  { key: 'nummer', label: 'Nummer', type: 'number' },
  { key: 'titel', label: 'Titel', type: 'string/text' },
  { key: 'beschreibung', label: 'Beschreibung', type: 'string/textarea' },
];

const ENTITY_TABS = [
  { key: 'hallo_welt_eintrag', label: 'Hallo Welt Eintrag', pascal: 'HalloWeltEintrag' },
] as const;
...
export default function AdminPage() {
  ...
  const [activeTab, setActiveTab] = useState<EntityKey>('hallo_welt_eintrag');
  const [selectedIds, setSelectedIds] = useState<Record<EntityKey, Set<string>>>(() => ({
    'hallo_welt_eintrag': new Set(),
  }));
  ...
  const getRecords = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'hallo_welt_eintrag': return (data as any).halloWeltEintrag as HalloWeltEintrag[] ?? [];
      default: return [];
    }
  }, [data]);
  ...
  const getFieldMeta = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'hallo_welt_eintrag': return HALLOWELTEINTRAG_FIELDS;
      default: return [];
    }
  }, []);
  ...
  const getServiceMethods = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'hallo_welt_eintrag': return {
        create: (fields: any) => LivingAppsService.createHalloWeltEintragEntry(fields),
        update: (id: string, fields: any) => LivingAppsService.updateHalloWeltEintragEntry(id, fields),
        remove: (id: string) => LivingAppsService.deleteHalloWeltEintragEntry(id),
      };
      default: return null;
    }
  }, []);
  ...
  return (
    ...
    >
      ...
      {(createEntity === 'hallo_welt_eintrag' || dialogState?.entity === 'hallo_welt_eintrag') && (
        <HalloWeltEintragDialog
          open={createEntity === 'hallo_welt_eintrag' || dialogState?.entity === 'hallo_welt_eintrag'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'hallo_welt_eintrag' ? handleUpdate : (fields: any) => handleCreate('hallo_welt_eintrag', fields)}
          defaultValues={dialogState?.entity === 'hallo_welt_eintrag' ? dialogState.record?.fields : undefined}
          enablePhotoScan={AI_PHOTO_SCAN['HalloWeltEintrag']}
          enablePhotoLocation={AI_PHOTO_LOCATION['HalloWeltEintrag']}
        />
      )}
      {viewState?.entity === 'hallo_welt_eintrag' && (
        <HalloWeltEintragViewDialog
          open={viewState?.entity === 'hallo_welt_eintrag'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
          onEdit={(r: any) => { setViewState(null); setDialogState({ entity: 'hallo_welt_eintrag', record: r }); }}
        />
      )}
