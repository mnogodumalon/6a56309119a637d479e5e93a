...
import type { HalloWeltEintrag, CreateHalloWeltEintrag } from '@/types/app';
...
export class LivingAppsService {
  // --- HALLO_WELT_EINTRAG ---
  static async getHalloWeltEintrag(): Promise<HalloWeltEintrag[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.HALLO_WELT_EINTRAG}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec
    })) as HalloWeltEintrag[];
    return enrichLookupFields(records, 'hallo_welt_eintrag');
  }
  static async getHalloWeltEintragEntry(id: string): Promise<HalloWeltEintrag | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.HALLO_WELT_EINTRAG}/records/${id}`);
    const record = { record_id: data.id, ...data } as HalloWeltEintrag;
    return enrichLookupFields([record], 'hallo_welt_eintrag')[0];
  }
  static async createHalloWeltEintragEntry(fields: CreateHalloWeltEintrag) {
    return callApi('POST', `/apps/${APP_IDS.HALLO_WELT_EINTRAG}/records`, { fields: cleanFieldsForApi(fields as any, 'hallo_welt_eintrag') });
  }
  static async updateHalloWeltEintragEntry(id: string, fields: Partial<CreateHalloWeltEintrag>) {
    return callApi('PATCH', `/apps/${APP_IDS.HALLO_WELT_EINTRAG}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'hallo_welt_eintrag') });
  }
  static async deleteHalloWeltEintragEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.HALLO_WELT_EINTRAG}/records/${id}`);
  }
